...
def create_map(fn=osm_fn):
    ...
    for entity in osmread.parse_file(str(fn)):
        if isinstance(entity, osmread.Way):
            tags = entity.tags
            if 'highway' in tags \
                and not (tags['highway'] in ['footway', 'cycleway', 'bridleway',
                                             'path', 'bus_guideway', 'track']) \
                ...
                and not ('service' in tags and tags['service'] in ['parking_aisle', 'driveway']) \
...
def test_path1():
    ...
    states_sol = [(2963305939, 249348325), (2963305939, 249348325), (2963305939, 249348325),
                  (2963305939, 249348325), (2963305939, 249348325), (2963305939, 249348325),
                  (2963305939, 249348325), (249348325, 1545679243), (1545679243, 3663115134),
                  (1545679243, 3663115134), (1545679243, 3663115134), (3663115134, 1545679251),
                  (1545679251, 20910628), (1545679251, 20910628), (20910628, 1573474889),
                  (20910628, 1573474889), (20910628, 1573474889)]
    assert states == states_sol, f"Got states: {states}"


def test_path2_proj():
    prepare_files()
    map_con_latlon = create_map2()
    ...
    matcher = DistanceMatcher(map_con, max_dist=300, max_dist_init=25,
                              obs_noise=50, obs_noise_ne=75, min_prob_norm=0.01,
                              dist_noise=50,
                              max_lattice_width=5,
...
def test_path2():
    ...
    # track = mm.util.interpolate_path(track, 5)
    map_con = create_map2()
    matcher = DistanceMatcher(map_con, max_dist=100, max_dist_init=20,
